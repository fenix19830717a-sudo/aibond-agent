...
class AibondClient:
    ...
    async def list_my_tasks(self, status: str = None):
        ...
        # Bug #8 fix: use asyncio.to_thread to avoid blocking event loop
        data = await asyncio.to_thread(self._sync_urlopen_json, req)
        agent_id = data["id"]

        tasks_url = f"{base_url}/api/agents/{agent_id}/tasks"
        if status:
            tasks_url += f"?status={urllib.parse.quote(status)}"
        # Bug #9 fix: add Authorization header
        req2 = urllib.request.Request(tasks_url, headers={"Authorization": f"Bearer {self.token}"})
        return await asyncio.to_thread(self._sync_urlopen_json, req2)
    ...
    async def get_session_info(self, session_id: str):
        ...
        return await asyncio.to_thread(self._sync_urlopen_json, req)

    @staticmethod
    def _sync_urlopen_json(req) -> dict:
        """Blocking helper: open URL and parse JSON response."""
        import json as _json
    ...
    async def _recv_loop(self, ws: ClientConnection) -> None:
        ...
        try:
            async for raw in ws:
                ...
                # Put into queue for async consumers (skip if queue disabled)
